...
const PROGRESS_KEY = "wf-tracker:progress:v4";
const OLD_PROGRESS_KEYS = ["wf-tracker:progress:v3", "wf-tracker:progress:v2", "wf-tracker:progress:v1"];
const PAGE_SIZE = 120;
...
const CATEGORIES = {
  warframes: {
    label: "Warframes",
    item: "Warframe",
    subs: [["all", "All"], ["warframes", "Warframes"], ["prime-warframes", "Prime Warframes"], ["archwings", "Archwings"], ["necramechs", "Necramechs"]],
  },
  weapons: {
    label: "Weapons",
    item: "Weapon",
    subs: [["all", "All"], ["primary", "Primary"], ["secondary", "Secondary"], ["melee", "Melee"], ["archguns", "Archguns"], ["archmelee", "Archmelee"], ["sentinel-weapons", "Sentinel Weapons"], ["amps", "Amps"], ["kitguns", "Kitguns"], ["zaws", "Zaws"], ["other", "Other"]],
  },
  companions: {
    label: "Companions",
    item: "Companion",
    subs: [["all", "All"], ["sentinels", "Sentinels"], ["kubrows", "Kubrows"], ["kavats", "Kavats"], ["moa", "MOA"], ["hounds", "Hounds"], ["predasites", "Predasites"], ["vulpaphylas", "Vulpaphylas"], ["robotic", "Robotic"], ["beasts", "Beasts"], ["other", "Other"]],
  },
};
...
function makeOwnedBlueprintList(item) {
  ...
  if (item.category === "warframes" && ["warframes", "prime-warframes"].includes(item.subcategory)) {
    FRAME_PARTS.forEach((framePart) => {
      ...
      const requirements = frameComponentRequirements(component);
      addOwnedPart(parts, {
...
function frameComponentRequirements(component) {
  ...
  if (!hasOrokinCell) {
    requirements.push({
      ...
      kind: "Resource",
      count: 1,
      notes: "Added fallback because the API may omit this common Warframe component material.",
      drops: [],
      requirements: [],
    });
  }
...
function renderDetails() {
  ...
  if (!item) {
    ...
    el.details.innerHTML = `<div class="empty-state"><h2>Select an item</h2><p>Choose a category and item to see owned blueprints and crafting requirements.</p></div>`;
    return;
  }
  ...
  el.details.innerHTML = `
    ...
    <p class="details-help">Only the checkbox rows count toward completion. Crafting requirements are shown directly under each blueprint/part.</p>
  ...
  return `
    <label class="part-row ${checked ? "is-checked" : ""}">
      ...
      <span>
        ...
        ${visibleRequirements(part.requirements)}
      </span>
    </label>`;
}

function visibleRequirements(requirements) {
  if (!requirements?.length) return `<p class="drops">Requires: no crafting requirements found in the API.</p>`;
  return `<div class="requirement-list requirement-list--inline"><strong>Requires</strong>${requirementList(requirements)}</div>`;
}
